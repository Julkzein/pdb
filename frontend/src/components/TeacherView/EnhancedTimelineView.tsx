...
const EnhancedTimelineView: React.FC = () => {
  ...
  const handleExport = () => {
    if (!enhancementData || !teacherContext || !graphState) return;

    // Build markdown document
    let markdown = `# Teaching Plan: ${teacherContext.subject}\n\n`;
    markdown += `**Age Group:** ${teacherContext.ageGroup}\n`;
    markdown += `**Total Duration:** ${graphState.totTime} minutes\n`;
    markdown += `**Difficulty:** ${enhancementData.overallSuggestions.estimatedDifficulty}\n\n`;

    markdown += `## Overall Suggestions\n\n`;
    markdown += `### Age Adaptations\n${enhancementData.overallSuggestions.ageAdaptations}\n\n`;
    markdown += `### Subject Context\n${enhancementData.overallSuggestions.subjectContext}\n\n`;

    markdown += `## Activity Sequence\n\n`;

    graphState.activities.forEach((activity, idx) => {
      const enhancement = enhancementData.activityEnhancements.find(e => e.activityPosition === idx);
      if (!enhancement) return;

      markdown += `### ${idx + 1}. ${activity.activityName} (${activity.time} min - ${getPlaneName(activity.plane)})\n\n`;
      markdown += `**Concrete Example:**\n${enhancement.concreteExample}\n\n`;
      markdown += `**Teaching Tips:**\n${enhancement.teachingTips}\n\n`;

      if (enhancement.timeManagement) {
        markdown += `**Time Management:**\n${enhancement.timeManagement}\n\n`;
      }

      if (enhancement.resources.length > 0) {
        markdown += `**Resources:**\n`;
        enhancement.resources.forEach(resource => {
          markdown += `- ${resource.title} (${resource.type})\n`;
          if (resource.description) markdown += `  ${resource.description}\n`;
          if (resource.suggestedUrl) markdown += `  ${resource.suggestedUrl}\n`;
        });
        markdown += `\n`;
      }
    });

    if (enhancementData.additionalResources.length > 0) {
      markdown += `## Additional Resources\n\n`;
      enhancementData.additionalResources.forEach(resource => {
        markdown += `- **${resource.title}** (${resource.type})\n`;
        if (resource.description) markdown += `  ${resource.description}\n`;
        if (resource.suggestedUrl) markdown += `  ${resource.suggestedUrl}\n`;
        markdown += `\n`;
    ...
    if (enhancementMetadata) {
      markdown += `\n---\n\n`;
      markdown += `*Generated by ${enhancementMetadata.model} (${enhancementMetadata.provider})*\n`;
    }
    ...
    link.href = url;
    link.download = `teaching-plan-${teacherContext.subject.replace(/\s+/g, '-').toLowerCase()}.md`;
    link.click();
